...
# a, b = get_data_boundaries(config)
# print("First entry:", a)
# print("Last entry:", b)
df = load_tick_data(startDateTime, endDateTime, config)
get_log_returns = TimeSeriesManipulation.getLogReturns(df['Close'].tolist())
#calculate_autocorr = TimeSeriesAnalysis.calculate_autocorrelation(get_log_returns, max_lags=int(len(df['Close'])/3.0), plot=True)
