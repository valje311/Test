...
class VideoMaker:
    ...
    def create_video(self, source_dir: str, output_file: str, fps: int = 30, 
        ...
        try:
            ...
            # Target Resolution anwenden wenn gegeben
            ...
            fourcc = cv2.VideoWriter.fourcc(*'mp4v')  # Korrigierte CV2-Syntax
            out = cv2.VideoWriter(output_file, fourcc, fps, (width, height))
            
            if not out.isOpened():
                if self.status_callback:
                    self.status_callback(f"❌ Cannot create video file: {output_file}")
                return False
            ...
            for i, (image_path, image_id) in enumerate(image_files):
                ...
                if target_resolution or (current_height, current_width) != (height, width):
