...
def createCandleDataFrame(df, config, plots_dir=None) -> pd.DataFrame:
    ...
    Args:
        ...
        config (ConfigDict): Configuration object with get() method
    ...
    timestamp_col = config.get('SQL', 'TimeColName', fallback='Timestamp')
    ...
    with tqdm(total=5, desc="Creating candles") as pbar:
        ...
        time_granularity = config.get('Candle', 'TimeGranularity', fallback='30s')
        df['group_timestamp'] = df[timestamp_col].dt.floor(time_granularity)
        pbar.update(1)
        ...
        table_name = config.get('SQL', 'TableName', fallback='Trading Data')
        mpf.plot(
            plot_df,
            type='candle',
            volume=True,
            title=f"{table_name} Candlestick Chart",
