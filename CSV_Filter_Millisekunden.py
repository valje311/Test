...
class CSVToolApp:
    def __init__(self, root):
        ...
        self.root.geometry("1200x800")  # Größere Startgröße für 4K-Displays

        # Modernes Theme anwenden
        style = ttk.Style()
        style.theme_use('clam')  # 'clam', 'alt', 'default', 'classic'

        # Schriftgröße für alle Elemente erhöhen
        default_font = ('Segoe UI', 11)  # Standardschriftart und -größe
        heading_font = ('Segoe UI', 12, 'bold')  # Für Überschriften

        # Benutzerdefinierte Schriftarten auf verschiedene Widget-Typen anwenden
        ...
        # Größere Zeilen in Treeview (Datenvorschau)
    ...
    def _create_column_management_ui(self, parent):
        ...
        # Listbox für die Spaltenauswahl mit größerer Schrift
        self.columns_listbox = tk.Listbox(
            frame, 
            selectmode=tk.EXTENDED, 
            exportselection=0,
            font=('Segoe UI', 11),  # Angepasste Schriftgröße
            height=15  # Mehr sichtbare Einträge
        )
    ...
    def _create_export_options_ui(self, parent):
        ...
        ttk.Label(frame, text="Format:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
        self.export_format_var = tk.StringVar(value='CSV')
        export_format_combo = ttk.Combobox(frame, textvariable=self.export_format_var, 
                                          values=["CSV", "SQL"], width=5)
        export_format_combo.grid(row=1, column=1, padx=5, pady=5, sticky="w")
        
        # SQL-spezifische Optionen
        self.sql_frame = ttk.LabelFrame(frame, text="SQL-Export Optionen", padding="5")
        self.sql_frame.grid(row=2, column=0, columnspan=4, padx=5, pady=5, sticky="ew")
        self.sql_frame.grid_remove()  # Standardmäßig ausblenden
        
        # Tabellenname für SQL
        ttk.Label(self.sql_frame, text="Tabellenname:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.sql_table_var = tk.StringVar(value='exported_data')
        ttk.Entry(self.sql_frame, textvariable=self.sql_table_var, width=20).grid(row=0, column=1, padx=5, pady=5, sticky="w")
        
        # Zeigen/Verstecken der SQL-Optionen basierend auf der Formatauswahl
        def on_format_change(*args):
            if self.export_format_var.get() == 'SQL':
                self.sql_frame.grid()
            else:
                self.sql_frame.grid_remove()
        
        self.export_format_var.trace('w', on_format_change)
        
        # Formatierungsoptionen für Forex-Daten
        self.forex_frame = ttk.LabelFrame(frame, text="Forex-Daten Optionen", padding="5")
        self.forex_frame.grid(row=3, column=0, columnspan=4, padx=5, pady=5, sticky="ew")
        ...
        # Export-Buttons
        self.export_button = ttk.Button(frame, text="Auswahl Exportieren", command=self.export_data, state=tk.DISABLED)
        self.export_button.grid(row=4, column=0, columnspan=2, padx=5, pady=10, sticky="ew")
        
        self.export_vis_button = ttk.Button(frame, text="Visualisierung", command=self.show_visualization_options, state=tk.DISABLED)
        self.export_vis_button.grid(row=4, column=2, columnspan=2, padx=5, pady=10, sticky="ew")
    ...
    def export_data(self):
        ...
        # 2. Exportformat bestimmen
        export_format = self.export_format_var.get()
        file_extension = ".sql" if export_format == "SQL" else ".csv"
        filetypes = [("SQL Dateien", "*.sql"), ("Alle Dateien", "*.*")] if export_format == "SQL" else [
            ("CSV Dateien", "*.csv"), ("Textdateien", "*.txt"), ("Alle Dateien", "*.*")]

        # 3. Speicherort abfragen
        export_path = filedialog.asksaveasfilename(
            title=f"Ausgewählte Daten als {export_format} speichern unter...",
            defaultextension=file_extension,
            filetypes=filetypes,
            initialfile=f"export_{os.path.basename(self.filepath)}" if self.filepath else f"export{file_extension}"
        )
        if not export_path:
            return

        # 4. Export-Einstellungen holen
        ...
        try:
            # 5. DataFrame für den Export vorbereiten
            ...
            # 6. Dezimalstellen anwenden (auf numerische Spalten im Export-DF)
            ...
            # 7. Export basierend auf dem gewählten Format
            if export_format == "SQL":
                self.export_to_sql(export_df, export_path)
            else:
                # CSV-Export (bestehender Code)
                export_df.to_csv(
                    export_path,
                    sep=export_sep,
                    decimal=export_dec,
                ...
                self.set_status(f"CSV-Datei erfolgreich gespeichert: {os.path.basename(export_path)}")
                
            messagebox.showinfo("Erfolg", f"Ausgewählte Daten wurden erfolgreich im {export_format}-Format nach\n{export_path}\ngespeichert.")

        except Exception as e:
            self.set_status(f"Fehler beim {export_format}-Export.")
            messagebox.showerror("Exportfehler", f"Ein Fehler ist beim Speichern der Datei aufgetreten:\n{str(e)}")

    def export_to_sql(self, df, filepath):
        """
        Exportiert einen DataFrame als SQL-Skript mit CREATE TABLE und INSERT-Anweisungen.
        
        Args:
            df: Der zu exportierende DataFrame
            filepath: Zielpfad für die SQL-Datei
        """
        table_name = self.sql_table_var.get().strip()
        if not table_name:
            table_name = "exported_data"
        
        # SQL-Datentypen aus Pandas-Datentypen ableiten
        def get_sql_type(dtype, sample_value=None):
            if pd.api.types.is_integer_dtype(dtype):
                ...
                # Für Strings, prüfen wir die Länge der längsten Zeichenkette
                if sample_value is not None:
                    try:
                        max_len = df[sample_value].astype(str).str.len().max()
                        return f"VARCHAR({max(max_len, 50)})"  # Mindestens 50 Zeichen
                    except:
                        pass
                return "TEXT"
    
        with open(filepath, 'w', encoding='utf-8') as f:
            # 1. DROP TABLE IF EXISTS hinzufügen
            f.write(f"-- Generiert von CSV Filter Millisekunden Tool am {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write(f"DROP TABLE IF EXISTS `{table_name}`;\n\n")
            
            # 2. CREATE TABLE-Statement erstellen
            ...
            for col in df.columns:
                ...
                # SQL-Identifier escaped mit Backticks
                columns.append(f"    `{col}` {sql_type}")
            
            f.write(",\n".join(columns))
            f.write("\n);\n\n")
            
            # 3. INSERT-Statements erstellen
            f.write(f"-- Daten für Tabelle `{table_name}`\n")
            batch_size = 1000  # Anzahl der Zeilen pro INSERT-Statement
            
            # DataFrame in Batches aufteilen für mehrere INSERT-Statements
            total_rows = len(df)
            batches = (total_rows + batch_size - 1) // batch_size  # Aufrunden
            ...
            for batch in range(batches):
                ...
                # Startet das INSERT-Statement
                f.write(f"INSERT INTO `{table_name}` (`")
                f.write("`, `".join(df.columns))
                f.write("`) VALUES\n")
                
                # Werte für jede Zeile formatieren
                rows = []
                for _, row in batch_df.iterrows():
                    ...
                    for col in df.columns:
                        ...
                        if pd.isna(value):
                            ...
                            # String-Werte mit einfachen Anführungszeichen umschließen und escapen
                            # Einfache Anführungszeichen im Wert durch doppelte einfache Anführungszeichen ersetzen
        ...
        self.set_status(f"SQL-Datei erfolgreich erstellt: {os.path.basename(filepath)}")

if __name__ == "__main__":
    # Tkinter-Root-Fenster erstellen
    root = tk.Tk()
    
    # Bessere Skalierung auf hochauflösenden Displays (Windows)
    try:
